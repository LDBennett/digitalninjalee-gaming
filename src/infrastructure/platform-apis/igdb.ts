...
export function mapIgdbToMoods(
  ...
  for (const id of data.genreIds)   for (const m of GENRE_MOODS[id]     ?? []) moods.add(m);
  for (const id of data.themeIds)   for (const m of THEME_MOODS[id]     ?? []) moods.add(m);
  for (const id of data.gameModeIds) for (const m of GAME_MODE_MOODS[id] ?? []) moods.add(m);
  return [...moods];
}
...
async function getAccessToken(clientId: string, clientSecret: string): Promise<string> {
  ...
  if (!res.ok) throw new Error(`Twitch auth failed: ${res.status} ${res.statusText}`);
...
export async function fetchIgdbGameData(
  ...
  if (!res.ok) throw new Error(`IGDB API responded with ${res.status} ${res.statusText}`);
...
export async function createIgdbClient(clientId: string, clientSecret: string) {
  ...
  return {
    fetchGame: (title: string) => fetchIgdbGameData(title, clientId, accessToken),
  };
}
