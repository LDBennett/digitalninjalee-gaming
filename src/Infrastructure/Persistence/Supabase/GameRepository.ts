...
import {
  GameRepository,
  GameFilter,
} from "@/src/Domain/Repositories/GameRepository";
import { GameState } from "@/src/Domain/Models/Game";
import { Result, ok, err } from "@/lib/result";
import {
  gameRowToDomain,
  gameStateToRow,
  gameMoodJunctionRows,
} from "@/src/Infrastructure/Persistence/Supabase/Mappers/GameMapper";
import { GameRowWithMoods } from "@/src/Infrastructure/Persistence/Supabase/Types";

const GAME_SELECT = "*, game_moods ( moods (*) )";

// This Repository are the functions that directly interact with the database (findAll, findById, save, update, delete).
  ...
  return {
    ...
    async save(game: GameState): Promise<Result<void, Error>> {
      ...
      if (junctionRows.length > 0) {
        const { error: moodError } = await client
          .from("game_moods")
          .insert(junctionRows);
