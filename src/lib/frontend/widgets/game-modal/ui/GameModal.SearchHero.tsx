...
export function GameModalSearchHero({
  ...
  const hasSelectedGame = Boolean(form.coverArtUrl || form.backgroundUrl || form.igdbId);

  return (
    <div className="flex flex-col items-center justify-center py-6 px-4 text-center">
      <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-brand-900/40 text-brand-400 ring-1 ring-brand-700/50">
        <Sparkles size={28} />
      </div>
      ...
      {!hasSelectedGame && (
        <div className="mt-6 border-t border-gray-800/80 pt-4 w-full max-w-md">
          <button
            type="button"
            onClick={onContinueToDetails}
            className="flex items-center justify-center gap-1.5 w-full text-xs text-gray-400 hover:text-white transition-colors"
          >
            <PenLine size={13} />
