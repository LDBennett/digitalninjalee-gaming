...
const PLATFORM_ICON_LABELS: Record<Platform, string> = {
  pc: "PC (Steam)",
  xbox: "Xbox",
...
function SwitchMark({ className }: { className: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      className={className}
...
function PlatformMark({
  ...
  switch (platform) {
    case "pc":
      return <SteamMark className={className} />;
    case "xbox":
      return <XboxMark className={className} />;
    case "playstation":
      return <PlayStationMark className={className} />;
    case "switch":
      return <SwitchMark className={className} />;
    case "other":
...
export function PlatformIcon({
  ...
  const label = PLATFORM_ICON_LABELS[platform];
  return (
    <span title={label} className="inline-flex shrink-0 items-center">
      <PlatformMark
        platform={platform}
        className={cn(className, PLATFORM_ICON_COLORS[platform])}
      />
