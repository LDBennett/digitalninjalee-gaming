  ...
  return (
    <div className="relative">
      <label className="block mb-1 font-medium text-gray-400 text-xs">
        Title *
        {!isEditing && <span className="ml-1 text-gray-600">— search to auto-fill cover art</span>}
      </label>
      <div className="relative">
        <input
          type="text"
        ...
        />
        {searchLoading && (
          <span className="top-1/2 right-3 absolute text-gray-500 text-xs -translate-y-1/2">…</span>
        )}
      </div>
        ...
        <ul className="z-10 absolute bg-gray-800 shadow-xl mt-1 border border-gray-700 rounded-lg w-full overflow-hidden">
          {results.map((game) => (
            <li key={game.id}>
              ...
              >
                {game.coverUrl ? (
                  <img src={game.coverUrl} alt="" className="rounded w-10 h-10 object-cover shrink-0" />
                ) : (
                  <div className="bg-gray-700 rounded w-10 h-10 shrink-0" />
                )}
                <div className="min-w-0">
                  <p className="font-medium text-white text-sm truncate">{game.name}</p>
                  {game.released && <p className="text-gray-500 text-xs">{game.released.slice(0, 4)}</p>}
                </div>
              </button>
            </li>
          ))}
