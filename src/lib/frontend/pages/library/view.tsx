...
const TABS: LibraryTab[] = ["all", "completed", "main-complete", "ongoing", "dropped"];

export function LibraryView() {
  const {
    games, filtered, paginated, page, setPage, totalPages,
    moods, tab, setTab, searchQuery, setSearchQuery, moodFilter, setMoodFilter,
    sortBy, setSortBy, platformFilter, setPlatformFilter,
    editGame, setEditGame, gamesLoading, isAuthenticated,
    handleStatusChange, handleEdit, handleDelete, handleAdd, showAdd, setShowAdd,
  } = useLibrary();
  ...
  return (
    <div ref={topRef} className="mx-auto max-w-3xl">
      <div className="mb-6">
        ...
        <TabBar tabs={TABS} value={tab} onChange={setTab} labels={LIBRARY_TAB_LABELS} className="mb-5" />
      ...
      {gamesLoading ? (
        <div className="space-y-3 md:space-y-5">{Array.from({ length: 5 }).map((_, i) => <GameCardSkeleton key={i} />)}</div>
      ) : (
        <>
          {filtered.length > 0 && (
            <p className="mb-3 text-gray-600 text-sm">{filtered.length} {LIBRARY_TAB_LABELS[tab].toLowerCase()} game{filtered.length !== 1 ? "s" : ""}</p>
          )}
          <GameCardList
            games={paginated}
            emptyState={
      ...
      {showAdd && <AddGameModal isOpen onClose={() => setShowAdd(false)} onSave={handleAdd} moods={moods} />}
    </div>
  );
}
