...
import { GameCard, GameCardList, GameStatsGrid } from "@/src/lib/frontend/entities/game";
...
const TAB_LABELS: Record<DashboardTab, string> = { playing: "Currently Playing", backlog: "Backlog" };
...
export function DashboardView() {
  ...
  const {
    ...
    loading, isAuthenticated, handleAdd, handleStatusChange,
  } = useDashboard();
    ...
    <div className="mx-auto max-w-5xl">
      <div className="flex sm:flex-row flex-col justify-between sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="mt-0.5 text-gray-500 text-sm">{stats.total} games tracked</p>
        </div>
        {isAuthenticated && (
          <div className="flex gap-3">
            <button onClick={() => setShowPicker(true)} className="flex flex-1 sm:flex-none justify-center items-center gap-2 bg-linear-to-r from-brand-950 hover:from-brand-800 to-brand-800 hover:to-brand-600 shadow-lg px-4 py-2 rounded-lg font-semibold text-white text-sm text-center transition-all">
              <Dices /> Random Game
            </button>
          </div>
        )}
