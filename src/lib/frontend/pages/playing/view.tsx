...
const TAB_LABELS: Record<PlayingTab, string> = {
  playing: "Playing", ongoing: "Ongoing", replaying: "Replaying",
};
const EMPTY_STATE = {
  playing:   { heading: "Nothing in progress",       hint: "Head to your Backlog to start a game." },
  ongoing:   { heading: "No ongoing games",           hint: "Move a game to Ongoing from the playing page." },
  replaying: { heading: "No games being replayed",    hint: "Mark a completed game as 'Replaying' to see it here." },
} as const;

export function PlayingView() {
  const {
    activeTab, setActiveTab, filtered, paginated, page, setPage, totalPages,
    moods, moodFilter, setMoodFilter, sortBy, setSortBy, platformFilter, setPlatformFilter,
    searchQuery, setSearchQuery, editGame, setEditGame,
    loading, isAuthenticated, handleStatusChange, handleEdit, handleDelete,
  } = usePlaying();

  const topRef = useScrollToTop(page);
  const [showFilters, setShowFilters] = useState(false);
  const activeFilterCount = [moodFilter !== null, platformFilter !== null, sortBy !== "priority-desc"].filter(Boolean).length;
  const emptyState = EMPTY_STATE[activeTab];
  const countLabel = activeTab === "playing" ? "active game" : activeTab === "ongoing" ? "ongoing game" : "replaying game";
  ...
  return (
    <div ref={topRef} className="mx-auto max-w-3xl">
      <div className="mb-6">
        ...
        <p className="mt-0.5 text-gray-500 text-sm">{filtered.length} {countLabel}{filtered.length !== 1 ? "s" : ""}{moodFilter ? ` · ${moodFilter}` : ""}</p>
      </div>

      <TabBar tabs={TAB_VALUES} value={activeTab} onChange={setActiveTab} labels={TAB_LABELS} className="mb-6" />
      ...
      {showFilters && <GameFiltersPanel moods={moods} moodFilter={moodFilter} onMoodChange={setMoodFilter} sortBy={sortBy} onSortChange={setSortBy} platformFilter={platformFilter} onPlatformChange={setPlatformFilter} className="mb-5" />}
      ...
      {editGame && <AddGameModal isOpen onClose={() => setEditGame(null)} onSave={handleEdit} onDelete={isAuthenticated ? handleDelete : undefined} editGame={editGame} moods={moods} />}
    </div>
  );
}
